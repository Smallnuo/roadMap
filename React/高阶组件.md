...
高阶函数返回结果必须是一个新的React组件，新组件的JSX肯定包含作为参数的组件

```jsx
const EnhancedComponent = higherOrderComponent(WrappedComponent)
```

组件是将props转换成UI，高阶组件是将组件转换为另一个组件
...
- PP模式的作用
    - 操作props，在组件内部可以操作props，传递给WrappedComponent一些新的props
        ```jsx
        function PPHoc(WrappedComponent) {
            return class PP extends React.Component {
            render() {
